...
export default function ChatPage() {
  ...
  return (
    <div className="min-h-screen flex flex-col bg-bg relative" style={bgStyle}>
      ...
      {/* Messages */}
      <main className="flex-1 overflow-y-auto px-5 py-4 max-w-lg mx-auto w-full relative z-10">
      ...
      <footer className="sticky bottom-0 bg-surface/90 backdrop-blur-xl border-t border-white/5 safe-area-bottom relative z-10">
        <div className="flex items-end gap-2 px-4 py-3 max-w-lg mx-auto">
          <button
          ...
          <textarea
            ...
            rows={1}
            className="flex-1 bg-surface-light text-text text-[15px] rounded-xl px-4 py-3 resize-none border border-white/5 focus:border-primary/50 focus:outline-none placeholder:text-text-muted max-h-[120px]"
          />
          <button
            onClick={sendMessage}
            disabled={!input.trim() || isTyping}
            className="p-3 rounded-xl bg-primary text-white disabled:opacity-30 disabled:cursor-not-allowed hover:bg-primary-dark transition-colors shrink-0"
          >
            <Send size={22} />
          </button>
        </div>
